...
class TD3Agent:
    def __init__(
        ...
        self.imitation_episodes = 20
        self.hitl_episodes = 140
        self.last_episode=0
    ...
    def get_action_from_keys(self, is_imitation=False):
        """
        Mengonversi input keyboard (pressed_keys) menjadi aksi diskrit untuk LunarLander.
        ...
        # Tombol throttle utama (diskrit: 1 untuk naik, 0 untuk tidak ada throttle)
        if 'w' in self.pressed_keys:
            self.main_throttle = 1
        ...
        # Tombol throttle lateral (diskrit: 1 untuk ke kanan, -1 untuk ke kiri, 0 untuk tidak ada throttle)
        if 'a' in self.pressed_keys:
            self.lateral_throttle = 1  # Ke kanan
        elif 'd' in self.pressed_keys:
            self.lateral_throttle = -1  # Ke kiri
        else:
            if self.step_no_keyboard_now > 0 or is_imitation:
                self.lateral_throttle = 0  # Tidak ada throttle lateral
                dummy_lateral = True
            else:
        ...
        # Mengembalikan aksi diskrit
        return np.array([self.main_throttle, self.lateral_throttle], dtype=np.float16)
    ...
    def imitation_learning(self, action):
        ...
        if len(self.memory_B_human) > self.batch_size:
            ...
            # Q-value Dari state + action manusia
                ...
                # Mean Advantage loss 
                critic_1_loss = tf.reduce_mean(tf.square(Q1_human - Q1_policy))
                critic_2_loss = tf.reduce_mean(Q2_human - Q2_policy)
            # Update Critic
            critic_1_grad = tape.gradient(critic_1_loss, self.critic_1.trainable_variables)
            critic_2_grad = tape.gradient(critic_2_loss, self.critic_2.trainable_variables)
            self.critic_1_optimizer.apply_gradients(zip(critic_1_grad, self.critic_1.trainable_variables))
        ...
        for episode in tqdm(range(1, self.n_episodes + 1)):
            ...
            while not done:
                ...
                if episode <= self.imitation_episodes:
                    ...
                    self.count_ask_human[self.count_episode] = None
                ...
                if action_human is not None:
                    try:
                        self.count_human_help[episode] += 1
                    except:
                        self.count_human_help[episode] = 0
