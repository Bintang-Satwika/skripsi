...
from env_tanpawait_testing import FJSPEnv  # Your flexible job shop environment
...
class DDQN_model:
    def __init__(self,
        ...
        self.save_dir = os.path.join(self.current_dir, 'DQN_1')
        if not os.path.exists(self.save_dir):
            os.makedirs(self.save_dir)
        self.num_agents = 3
        self.state_dim = 9-self.num_agents+1
        self.action_dim = 3
        self.gamma = gamma
        self.lr = lr
        self.tau = tau
...
if __name__ == "__main__":
    ...
    for episode in tqdm(range(DDQN.episode_load+1, num_episodes + 1)):
        ...
        while not done:
            ...
            # proses per baris
            all_states_DQN = np.array([
                np.delete(all_states[i], removals[i])
                for i in range(len(removals))
            ])
            all_states_normalized = DDQN.normalize(all_states_DQN) # Normalize the state
            ...
            all_next_states_DQN = np.array([
                np.delete(all_next_states[i], removals[i])
                for i in range(len(removals))
            ])
            all_next_states_normalized = DDQN.normalize(all_next_states_DQN)
            ...
            for r in range(num_agents):
                if not np.array_equal(all_next_states[r], all_states[r]):
                    DDQN.update_RL_memory(r, all_states_normalized[r], all_actions[r], all_rewards[r], 
                                           all_next_states_normalized[r], done, all_mask_actions[r], all_mask_actions_next[r])
