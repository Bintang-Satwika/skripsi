...
class TD3Agent:
    def __init__(
        ...
        # Waktu timeout dan durasi minimum untuk aksi manusia
        self.human_action_timeout = 0.5  # 0.5 detik
        self.min_human_duration_steps = 2 # Minimal 2 detik aksi manusia
        self.last_action_time = time.time()  # Waktu terakhir aksi manusia
        self.human_duration_steps = time.time()  # Waktu  aksi manusia
    ...
    def get_action(self, keys):
        ...
        if keys[pygame.K_w]:
            ...
            # Kembali ke 0 secara perlahan jika durasi kurang dari timeout
            if current_time - self.last_action_time < self.human_action_timeout:
    ...
    def ask_human(self, state, action, reward_satu_episode):
        ...
        # Tentukan apakah manusia perlu membantu
        if len(self.Quen) > 0:
            if Is > max(self.Quen) and reward_satu_episode < self.reward_max / self.th:
                self.human_help = True
                #self.human_step_now = max(self.human_step_now, self.min_human_duration_steps)
                self.last_action_time = current_time
            else:
                self.human_help = False
        else:
    ...
    def train(self):
        ...
        for episode in tqdm(range(1, self.n_episodes + 1)):
            ...
            while not done:
                ...
                current_time = time.time()
                if (action_human is None and current_time - self.last_action_time >= self.human_action_timeout) or (current_time - self.human_duration_steps > self.min_human_duration_steps):
                    action_RL = self.select_action(state)
                    action = action_RL
                

                if current_time - self.human_duration_steps > self.min_human_duration_steps:
                    self.ask_human(state, action_RL, reward_satu_episode)

                #     self.min_human_duration_steps =2 # Minimal 2 detik aksi manusia
                #     self.ask_human(state, action_RL, reward_satu_episode)
                # else:
                #     self.min_human_duration_steps -=
