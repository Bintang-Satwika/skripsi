...
class DDQN_model:
    def __init__(self,
        ...
        # 1) Build the network & buffer dicts
        ...
        for r in range(self.num_agents):
            ...
            self.target_dqn_network_dict[r].set_weights(
                self.dqn_network_dict[r].get_weights())
            self.replay_buffer_dict[r] = deque(maxlen=buffer_length)

        # 2) NOW resume if requested
        self.episode_load = episode_load
        if self.episode_load > 0:
            self._resume_from_episode(self.episode_load)

        # 3) Then set up optimizers and anything else
        self.dqn_optimizer_dict = {}
        for r in range(self.num_agents):
            self.dqn_optimizer_dict[r] = tf.keras.optimizers.Adam(
                learning_rate=self.lr)

    def _resume_from_episode(self, episode: int):
        """Load weights, replay buffers, and reward history from disk."""
        # 1) Load network weights
        for r in range(self.num_agents):
            dqn_path = os.path.join(self.save_dir, f'dqn_agent_{r}_episode_{episode}.h5')
            tgt_path = os.path.join(self.save_dir, f'target_dqn_agent_{r}_episode_{episode}.h5')
            if not os.path.exists(dqn_path) or not os.path.exists(tgt_path):
                raise FileNotFoundError(f"Cannot find saved models for agent {r} at episode {episode}")
            self.dqn_network_dict[r].load_weights(dqn_path)
            self.target_dqn_network_dict[r].load_weights(tgt_path)

        print(f">>> Loaded DQN and target networks from episode {episode}")

        # 2) Load replay buffers
        for r in range(self.num_agents):
            buf_path = os.path.join(self.save_dir, f'replay_buffer_agent_{r}_episode_{episode}.pkl')
            if os.path.exists(buf_path):
                with open(buf_path, 'rb') as f:
                    self.replay_buffer_dict[r] = pickle.load(f)
            else:
                print(f"Warning: replay buffer for agent {r} not found at episode {episode}")

        # 3) Load cumulative rewards history (optional)
        rewards_path = os.path.join(self.save_dir, 'cumulative_rewards.json')
        if os.path.exists(rewards_path):
            with open(rewards_path, 'r') as f:
                all_rewards = json.load(f)
            # convert keys back to int
            self.cumulative_reward_episode = {int(k): v for k, v in all_rewards.items()}
            print(f">>> Loaded cumulative rewards up to episode {max(self.cumulative_reward_episode):d}")
        else:
            print("Warning: cumulative_rewards.json not found; starting fresh.")
