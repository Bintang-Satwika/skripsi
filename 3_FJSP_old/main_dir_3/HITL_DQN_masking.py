...
class DDQN_model:
    def __init__(self,
        ...
        self.save_dir = os.path.join(self.current_dir, 'model_HITL_Masking_4_60ep')
    ...
    def update_human_memory(self, state, action, reward, next_state, done, mask_actions):
        """Simpan (s, a) dari aksi manusia ke buffer terpisah."""
        self.replay_buffer_human.append((state, action, reward, next_state, done, mask_actions))
    ...
    def take_human_minibatch(self):
        ...
        return mb_states, mb_actions, mb_rewards, mb_next_states, mb_dones, mb_action_mask
    ...
    def train_double_dqn(self):
        ...
        if len(self.replay_buffer_human) > self.batch_size:
            mb_states_human, mb_actions_human, r, n, d, mb_human_mask = self.take_human_minibatch()
            mb_actions_human = tf.cast(mb_actions_human, tf.int32)
            mb_human_mask = tf.cast(mb_human_mask, tf.bool)
            # Menghitung Advantage loss
            with tf.GradientTape(persistent=True) as tape:
                # Q-value Dari state dan action manusia
                Q_values_RL=self.dqn_network(mb_states_human, training=True)
                mb_actions_predicted = tf.stop_gradient(tf.argmax(Q_values_RL, axis=2) )
                Q_human = tf.reduce_sum(Q_values_RL * tf.one_hot(mb_actions_human, self.action_dim), axis=2)

                Q_policy =tf.reduce_sum( Q_values_RL * tf.one_hot( mb_actions_predicted, self.action_dim),axis=2)
                # Advantage loss
    ...
    for episode in tqdm(range(1, num_episodes + 1)):
        ...
        while not done:
            ...
            if episode < DDQN.episode_hitl:
                ...
                human_actions, human_mask_actions = DDQN.human_in_the_loop(state, joint_actions, reward_episode,env, episode)
            ...
            if not np.array_equal(next_state, state):
                if human_actions is not None:
                    ...
                    DDQN.update_human_memory(state, human_actions, reward, next_state, np.repeat(done, num_agents), human_mask_actions)
                else:
                    try:
                    ...
                    DDQN.update_RL_memory(state, joint_actions, reward, next_state, np.repeat(done, num_agents), mask_actions, mask_actions_next)
