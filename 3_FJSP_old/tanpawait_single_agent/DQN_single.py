...
class DDQN_model:
    def __init__(self,
        ...
        self.save_dir = os.path.join(self.current_dir, 'DQN_single_1')
    ...
    def create_dqn_network(self):
        ...
        x = layers.TimeDistributed(layers.Dense(128, activation='relu'))(state_input)
    ...
    #     # For 3 agents: input shape becomes (num_agents, state_dim)
    #     state_input = layers.Input(shape=(self.num_agents, self.state_dim))
    #     state_input = layers.Flatten()(state_input)
    ...
    def masking_action_single(self, masking_action):
        # [Accept, Decline, Continue]
        # Example masking_action: [[True, False, False],
        #                         [False, True, False],
        #                         [True, False, False]]
        # Convert the boolean mask to a numpy array of integers (0 or 1)
        masking_action = np.array(masking_action, dtype=int)
        print("masking_action:", masking_action)
        # # Kelas 0: [0, 0, 0]
        # # Kelas 1: [0, 0, 1]
        # # Kelas 2: [0, 1, 0]
        # # Kelas 3: [0, 1, 1]
        ...
        # }
        # # Convert the tuple to a class index using the mapping
        # class_index = class_mapping.get(tuple(masking_action), None)
        # print("class_index:", class_index)
        # if class_index is not None:
        #     return class_index
        # else:
        #     raise ValueError("Invalid masking action: {}".format(masking_action))
...
if __name__ == "__main__":
    ...
    for episode in tqdm(range(DDQN.episode_load+1, num_episodes + 1)):
        ...
        state_dqn =np.delete(state, np.s_[5:7], axis=1) 
        ...
        while not done:
            ...
            DDQN.iterasi += 1
            state_dqn =np.delete(state, np.s_[5:7], axis=1) 
            state_normalized = DDQN.normalize(state_dqn) # Normalize the state
            ...
            next_state_dqn=np.delete(next_state, np.s_[5:7], axis=1) 
            reward = np.mean(reward)
            next_state_normalized = DDQN.normalize(next_state_dqn)
            ...
            if not np.array_equal(next_state_dqn, state_dqn):
