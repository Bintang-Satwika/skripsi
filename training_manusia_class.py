...
class TD3Agent:
    def __init__(
        ...
        n=15,
        sigma_init=0.4,
        sigma_target=0.2,  # bisa Anda gunakan untuk mengecilkan noise sesuai kebutuhan
        noise_clip=0.5,
        seed=10,
        render_mode='human'
    ):
        ...
        # Set seed untuk reproduktifitas (opsional)
        random.seed(seed)
        np.random.seed(seed)
        ...
        #keyboard action lunar lander
        self.main_throttle = 0.0
        self.lateral_throttle = 0.0
        self.throttle_rate = 1

        # Bungkus environment untuk mendapatkan return dan length per episode
        self.env = gym.wrappers.RecordEpisodeStatistics(self.env, buffer_length=n_episodes)

        # Dapatkan dimensi state dan action
    ...
    def get_action(self, keys):
        ...
        # else:
        #     self.main_throttle = max(0.0, self.main_throttle - self.throttle_rate * 1)  # Kembali ke 0 secara perlahan
    ...
    def ask_human(self, state, action, reward_satu_episode):
        ...
        if len(self.Quen)>0:
            if Is>max(self.Quen) and reward_satu_episode<self.reward_max/self.th:
                ...
                keys = pygame.key.get_pressed()
               # keys = self.wait_for_human_action()
    ...
    def train(self):
        ...
        for episode in tqdm(range(1, self.n_episodes + 1)):
            ...
            while not done:
                self.iterasi += 1
                # Event handler pygame (opsional jika ingin menutup window)
                # for event in pygame.event.get():
                ...
                action_human = self.ask_human(state, action_RL, reward_satu_episode)
