...
def MASKING_action(states, env):
    ...
    for i, state in enumerate(states):
        ...
        if (state[env.state_first_job_operation_location[3]]in state[env.state_operation_capability_location]):
            is_job_in_capability_yr_3=True
        ...
        if is_status_accept:
            ...
            if is_status_idle and is_job_in_capability_yr_3:
                wait_yr_3_action=True
                decline_action=True
            ...
            if is_status_idle and (state[env.state_first_job_operation_location[0]]==0 
                                   and state[env.state_first_job_operation_location[1]]==0 
                                   and state[env.state_first_job_operation_location[2]]==0 
                                   and state[env.state_first_job_operation_location[3]]==0):
                continue_action=True
            elif is_status_idle and (not is_job_in_capability_yr or not is_job_in_capability_yr_1 or not is_job_in_capability_yr_2 or not is_job_in_capability_yr_3):
                decline_action=True
        ...
        mask_actions.append([accept_action, wait_yr_1_action, wait_yr_2_action, wait_yr_3_action, decline_action, continue_action])
...
def FCFS_action(states, env):
    ...
    for i, state in enumerate(states):
        if state[env.state_operation_now_location]==0:
            ...
            elif (state[env.state_first_job_operation_location[3]]!=0 and 
                  state[env.state_first_job_operation_location[3]] in state[env.state_operation_capability_location]):# ada job di yr-3
                actions.append(3) # wait yr-3
            elif np.array_equal(state[env.state_first_job_operation_location], [0, 0, 0]):
                actions.append(5) # continue
            else:
                actions.append(4) # Decline
