...
def MASKING_action(states, env):
    ...
    for i, state in enumerate(states):
        ...
        #[Accept, Wait yr-1,  Decline, Continue]
        accept_action=False
        ...
        if is_status_accept:
            ...
            if is_status_idle and (state[env.state_first_job_operation_location[0]]==0 and state[env.state_first_job_operation_location[1]]==0 ):
                continue_action=True
            elif is_status_idle and (not is_job_in_capability_yr or not is_job_in_capability_yr_1):
                decline_action=True
        ...
        mask_actions.append([accept_action, wait_yr_1_action, decline_action, continue_action])
    ...
    for i, state in enumerate(states):
        if state[env.state_operation_now_location]==0:
            ...
            if state[env.state_status_location_all[i]]==1:
                ...
                actions.append(3) # continue
            else:
                actions.append(2) # Decline
