...
from env_3 import FJSPEnv  # Your flexible job shop environment
from RULED_BASED import MASKING_action, HITL_action, FCFS_action, RANDOM_action



class PrioritizedReplayBuffer:
    def __init__(self, capacity, alpha=0.6, beta=0.4, beta_increment=1e-5, epsilon=1e-6, path=False):
        """
        ...
        if path:
            with open(path , 'rb') as f:
                dummy= pickle.load(f)
            self.capacity=dummy.capacity
            self.alpha= dummy.alpha
            self.beta= dummy.beta
            self.beta_increment= dummy.beta_increment
            self.epsilon= dummy.epsilon
            self.buffer=dummy.buffer
            self.priorities=dummy.priorities
        ...
        print('len(self.replay_buffer):', len(self.buffer))
...
class DDQN_model:
    def __init__(self,
        ...
        self.save_dir = os.path.join(self.current_dir, 'model_RANDOM_priority_25_meanreward')
        ...
        dir_location='model_RANDOM_priority_25_meanreward'
    ...
    def train_double_dqn(self):
        ...
        with tf.GradientTape() as tape:
            ...
            mb_Q_values_next = self.dqn_network(mb_next_states, training=True)
...
if __name__ == "__main__":
    ...
    for episode in tqdm(range(DDQN.episode_load+1, num_episodes + 1)):
        ...
        while not done:
            ...
            if len(DDQN.replay_buffer) >= DDQN.batch_size and DDQN.iterasi % DDQN.update_delay == 0:
                DDQN.train_double_dqn()
                DDQN.update_target_weights()
