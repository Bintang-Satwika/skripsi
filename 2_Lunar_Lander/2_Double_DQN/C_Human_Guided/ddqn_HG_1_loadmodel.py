...
import json
from ddqn_model import  DDQNLoader
...
class TD3Agent:
    def __init__(
        ...
        self.save_dir = 'HG_saved_models_1_part2'
     ...
     # Load model dan replay buffer
        self.episode_load= 280
        current_dir = os.path.dirname(os.path.abspath(__file__))
        load_parameter_dir = os.path.join(current_dir, 'HG_saved_models_1_part1')
        ...
        model_loader = DDQNLoader(load_dir=load_parameter_dir,state_dim=self.state_dim, action_dim=self.action_dim)
        self.dqn_network, self.target_dqn_network= model_loader.load_models(episode=self.episode_load)
    ...
    def train_double_dqn(self):
        ...
        with tf.GradientTape() as tape:
            ...
            loss_RL = tf.reduce_mean(tf.square(y - mb_Q_values))

            Q_values=self.dqn_network(mb_states_human, training=True)
            mb_actions_predicted = tf.argmax(Q_values, axis=1) 
            Q_human = tf.reduce_sum(Q_values * tf.one_hot(mb_actions_human, self.action_dim), axis=1)
            Q_policy =tf.reduce_sum( Q_values * tf.one_hot( mb_actions_predicted, self.action_dim),axis=1)
            advantage_loss= tf.reduce_mean(weight_human*tf.square(Q_human - Q_policy))
            total_loss = loss_RL + advantage_loss
            

        # Menghitung gradien dan mengupdate bobot model
        grads = tape.gradient(total_loss,  self.dqn_network.trainable_variables)
    ...
    def select_action(self, state):
        ...
        q_values = self.dqn_network(state_tensor, training=False)
    ...
    def imitation_learning(self, state):
        ...
        if len(self.memory_B_human) > self.batch_size :
            ...
            with tf.GradientTape() as tape:
                # Dapatkan logits dari dqn_network (bentuk: [batch_size, num_actions])
                logits = self.dqn_network(mb_states_human, training=True)
                
                # --- Perhitungan softmax secara manual ---
                # Untuk stabilitas numerik, kurangi dengan nilai maksimum per baris
                ...
                softmax_probs = exp_logits / sum_exp_logits  # Probabilitas per aksi
                
                # --- Ambil probabilitas yang sesuai dengan aksi demonstrasi ---
                # Buat indeks: [ [0, mb_actions_human[0]], [1, mb_actions_human[1]], ... ]
                batch_indices = tf.range(tf.shape(logits)[0])
                indices = tf.stack([batch_indices, mb_actions_human], axis=1)
                true_action_probs = tf.gather_nd(softmax_probs, indices)
                
                # --- Hitung cross entropy loss secara manual ---
                imitation_loss = tf.reduce_mean(-tf.math.log(true_action_probs + 1e-9))
            
            grads = tape.gradient(imitation_loss,  self.dqn_network.trainable_variables)
            self.dqn_optimizer.apply_gradients(zip(grads, self.dqn_network.trainable_variables))
            del tape
