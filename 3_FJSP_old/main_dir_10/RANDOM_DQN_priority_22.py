...
class DDQN_model:
    def __init__(self,
        ...
        self.save_dir = os.path.join(self.current_dir, 'model_RANDOM_priority_22')
    ...
    for episode in tqdm(range(DDQN.episode_load+1, num_episodes + 1)):
        ...
        while not done:
            ...
            if not np.array_equal(next_state, state):
                ...
                DDQN.update_RL_memory(state_normalized, joint_actions, np.repeat(np.mean(reward), num_agents), next_state_normalized, np.repeat(done, num_agents), mask_actions, mask_actions_next)
