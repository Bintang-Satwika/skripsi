...
from circular_conveyor_6 import CircularConveyor
...
class FJSPEnv(gym.Env):
    ...
    def reward_wait(self, observations, actions,  is_action_wait_succeed,k_wait=0.5):
        ...
        for i, agent in enumerate(self.agents):
            ...
            if (actions[i]==1 or actions[i]==2) and is_action_wait_succeed[i]:
                ...
                #print("agent.speed[operation]: ", agent.speed[operation])
                #print("(self.multi_agent_speed[:,operation]): ", self.multi_agent_speed[:,operation])
    ...
    def reward_complete(self, k_complete=2):
