...
class DDQN_model:
    def __init__(self,
        ...
        self.save_dir = os.path.join(self.current_dir, 'DQN_yr0_1_training_again')
    ...
    def _train_step(self, mb_states, mb_actions, mb_rewards, mb_next_states, mb_dones, mb_action_mask_next):
        with tf.GradientTape() as tape:
            # Q(s,a)
            q_vals = self.dqn_network(mb_states, training=True)  # (batch_size, num_agents, action_dim)
            # mb_actions # (batch_size, num_agents)
            # tf.one_hot(mb_actions, self.action_dim)  # (batch_size, num_agents, action_dim)
            q_taken = tf.reduce_sum(q_vals * tf.one_hot(mb_actions, self.action_dim), axis=2)  # (batch_size, num_agents)
            # Q'(s',a')
            q_next = self.target_dqn_network(mb_next_states, training=False) # (batch_size, num_agents, action_dim)
            neg_inf = tf.fill(tf.shape(q_next), -np.inf)
            q_next_masked = tf.where(mb_action_mask_next, q_next, neg_inf) 
            max_q_next = tf.reduce_max(q_next_masked, axis=2)  # (batch_size, num_agents)
            # Bellman target
