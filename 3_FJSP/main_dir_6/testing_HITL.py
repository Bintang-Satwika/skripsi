...
MODEL_DIR = os.path.join(PARENT_DIR,'main_dir_6', "model_HITL_Masking_12_2")
...
def run_env(num_episodes, render):
    ...
    file_path= os.path.join(CURRENT_DIR, "HITL_Testing_cumulative_rewards.json")
    with open(file_path, "w") as f:
        json.dump(rewards, f, indent=4)
    file_path= os.path.join(CURRENT_DIR, "HITL_Testing_makespan.json")
    with open(file_path, "w") as f:
        json.dump(makespan, f, indent=4)
    file_path= os.path.join(CURRENT_DIR, "HITL_Testing_energy.json")
    with open(file_path, "w") as f:
