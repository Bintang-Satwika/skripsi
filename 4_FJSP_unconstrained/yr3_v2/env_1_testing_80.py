...
class FJSPEnv(gym.Env):
    ...
    def __init__(self, window_size: int, num_agents: int, max_steps: int, episode: int):
        ...
        self.state_first_job_operation_location=[7,8,9, 10] # yr, yr-1, yr-2, yr-3
        self.state_second_job_operation_location=[11,12, 13, 14] # yr, yr-1, yr-2, yr-3
        self.state_pick_job_window_location=15
        #---------------------------------------------------------------------
        ...
        # Sehingga total 5 aksi: 0=ACCEPT, 1=WAIT yr-1. 2=WAit yr-2, 3=wait yr-3, 4=DECLINE, 5=CONTINUE
        # ---------------------------------------------------------------------
    ...
    def action_wait(self, observation, agent, i, status_location, actions):
        if observation[status_location]==0:
            ...
            if observation[self.state_first_job_operation_location[1]] !=0 and actions[i]==1:
                ...
                self.is_action_wait_succeed[i]=True
            elif observation[self.state_first_job_operation_location[3]] !=0 and actions[i]==3:
                observation[self.state_pick_job_window_location]=3
                #print("WAIT yr-3")
                self.is_action_wait_succeed[i]=True
    ...
    def update_state(self, observation_all,  actions):
        ...
        for i, agent in enumerate(self.agents):
            ...
            # print("observation[self.state_yr_location]: ", int(observation[self.state_yr_location]))
            # print("window_sections: ", window_sections)
        ...
        for i, agent in enumerate(self.agents):
            ...
            if (actions[i]==1 or actions[i]==2) and is_action_wait_succeed[i]:
                if actions[i]==1:
                    ...
                    factor_x=4.0
                    operation=int(observations[i, self.state_first_job_operation_location[3]]-1)
                else:
                    print("FAILED ACTION: actions is not 1 or 2 or 3")
