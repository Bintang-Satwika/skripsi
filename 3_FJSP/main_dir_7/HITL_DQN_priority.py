...
class PrioritizedReplayBuffer:
    def __init__(self, capacity, alpha=0.6, beta=0.4, beta_increment=1e-5, epsilon=1e-6):
        ...
        beta_increment: How much to increment beta after each sample
    ...
    def store(self, state, action, reward, next_state, done, mask_actions, mask_actions_next):
        ...
        state, next_state: shape (num_agents, state_dim)
        action, reward, done: shape (num_agents,) or scalars
        mask_actions, mask_actions_next: shape (num_agents, action_dim) booleans
        """
        # If the buffer is not full, append; otherwise overwrite.
        if self.next_idx >= len(self.buffer):
            self.buffer.append((state, action, reward, next_state, done, mask_actions, mask_actions_next))
            self.priorities.append(0)  # placeholder, updated below
        else:
            self.buffer[self.next_idx] = (state, action, reward, next_state, done, mask_actions, mask_actions_next)

        # New transition gets max priority so it's likely to be sampled soon.
    ...
    def sample(self, batch_size):
        ...
        Returns:
          ...
          indices:   the sampled indices, shape (batch_size,)
        """
        if len(self.buffer) == 0:
            raise ValueError("Cannot sample from an empty buffer!")

        # Only consider the valid portion of the buffer
        valid_size = len(self.buffer)
        # Compute probabilities
        priorities_np = np.array(self.priorities[:valid_size], dtype=np.float32)
        ...
        # Increment beta slightly
        self.beta = min(1.0, self.beta + self.beta_increment)

        # Compute importance-sampling weights
        # w_i = (1/(N * P(i)))^beta, then normalized by max weight
        N = valid_size
        p = probs[indices]
    ...
    def update_priorities(self, indices, td_errors):
        """
        After computing new TD errors, update the priorities.
        td_errors: array-like of shape (batch_size,)
        """
        for idx, err in zip(indices, td_errors):
            priority = (abs(err) + self.epsilon)
...
class DDQN_model:
    def __init__(self,
        ...
        self.save_dir = os.path.join(self.current_dir, 'model_HITL_priority_1')
    ...
    def train_double_dqn(self):
        ...
        # 9) Periodically update target network (soft update)
    ...
    for episode in tqdm(range(DDQN.episode_load+1, num_episodes + 1)):
        ...
        while not done:
            ...
            if episode < DDQN.episode_hitl:
                ...
                mask_actions= human_mask_actions if human_actions is not None else mask_actions
