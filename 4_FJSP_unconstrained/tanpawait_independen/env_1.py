...
class FJSPEnv(gym.Env):
    ...
    def __init__(self, window_size: int, num_agents: int, max_steps: int, episode: int):
        ...
        self.state_status_location_all=[4]
        self.state_first_job_operation_location=[5] # yr
        self.state_second_job_operation_location=[6] # yr
        #---------------------------------------------------------------------
        ...
        # Sehingga total 5 aksi: 0=ACCEPT, 1=DECLINE, 2=CONTINUE
        # ---------------------------------------------------------------------
    ...
    def update_state(self, observation_all,  actions):
        ...
        for i, agent in enumerate(self.agents):
            ...
            status_location=self.state_status_location_all[0]
    ...
    def step(self, actions):
        ...
        reward_agent_all=-0.5+reward_working_all+reward_step_all+reward_accept_all
    ...
    def reward_accept(self, observations, k_accept=0.0):
        ...
        for r, agent in enumerate(self.agents):
            obs=observations[r]
            if obs[self.state_status_location_all[0]]==1:
    ...
    def render(self):
       # print(f"Time Step: {self.step_count}")
        ...
        for a, agent in enumerate(self.agents):
            #print("self.conveyor.job_details:, ", self.conveyor.job_details)
            print(f"Status Agent {agent.id} at position {int(self.observation_all[a][0])}: {int(self.observation_all[a][self.state_status_location_all]) }")
            print("window product: ", agent.window_product, "\nworkbench: ", agent.workbench)
        #     if agent.processing_time_remaining>0:
        #         print("agent.processing_time_remaining: ", agent.processing_time_remaining)
        #     print("\n")
