...
class TD3Agent:
    def __init__(
        ...
        self.episode_load=400
        load_dir='D:\\KULIAH\\skripsi\\CODE\\skripsi\\double_dqn\\highwaymodel_2'
        replay_buffer_path = os.path.join(load_dir, f'replay_buffer_episode_{self.episode_load}.pkl')
        with open(replay_buffer_path , 'rb') as f:
            self.memory_B = pickle.load(f)
        print('len(self.memory_B):', len(self.memory_B))

        loader = DDQNLoader(state_dim=50, action_dim=5,load_dir=load_dir)
        self.dqn_network, self.target_dqn_network = loader.load_models(episode=self.episode_load)
        # Optimizer
